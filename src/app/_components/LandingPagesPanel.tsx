...
export function LandingPagesPanel({ projectId }: { projectId: string }) {
  ...
  const [plausible, setPlausible] = useState<PlausibleSettings | null>(null);
  ...
  const [plausibleSiteDraft, setPlausibleSiteDraft] = useState("");
  const [savingAnalytics, setSavingAnalytics] = useState(false);
  const [savingPlausible, setSavingPlausible] = useState(false);
  ...
  const loadPages = useCallback(async () => {
    ...
    setPlausible(body?.plausible ?? null);
    setGa4PropertyDraft(body?.analytics?.ga4PropertyId ?? "");
    setPlausibleSiteDraft(body?.plausible?.siteId ?? "");
  }, [projectId]);
  ...
  const savePlausibleSite = useCallback(async () => {
    const trimmed = plausibleSiteDraft.trim();
    if (trimmed.length > 255) {
      setError("Plausible site ID must be 255 characters or fewer.");
      return;
    }
    if (/^https?:\/\//i.test(trimmed)) {
      setError("Use the Plausible site ID/domain, not a full URL.");
      return;
    }

    setSavingPlausible(true);
    setError(null);
    const res = await fetch(`/api/projects/${projectId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ plausibleSiteId: trimmed || null }),
    });
    const body = (await res.json().catch(() => null)) as {
      error?: unknown;
      project?: { plausibleSiteId?: string | null };
    } | null;

    setSavingPlausible(false);
    if (!res.ok) {
      setError(apiErrorMessage(body, "Failed to save Plausible settings."));
      return;
    }

    setPlausible((prev) => {
      const apiConfigured = prev?.apiConfigured ?? false;
      const siteId = body?.project?.plausibleSiteId ?? (trimmed || null);
      return {
        siteId,
        apiConfigured,
        apiBaseUrl: prev?.apiBaseUrl ?? "https://plausible.io",
  ...
  return (
    <section id="landing-pages" className="scroll-mt-48">
      ...
      <PlausibleAnalyticsSettingsCard
        settings={plausible}
        siteId={plausibleSiteDraft}
        onSiteIdChange={setPlausibleSiteDraft}
        onSave={savePlausibleSite}
        saving={savingPlausible}
      />
...
function PlausibleAnalyticsSettingsCard({
  settings,
  siteId,
  onSiteIdChange,
  onSave,
  ...
  settings: PlausibleSettings | null;
  siteId: string;
  ...
  const savedValue = settings?.siteId ?? "";
  const hasChanges = trimmed !== savedValue;
  const valid = trimmed.length <= 255 && !/^https?:\/\//i.test(trimmed);

  let toneClass =
    "border-zinc-200 bg-zinc-50 text-zinc-700 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-300";
  let statusLabel = "Loading Plausible settings…";
  let helpText =
    "Optional fallback for UTM reporting. The destination site must already have Plausible installed, and this app queries visits/pageviews by the UTM values it generates.";

  if (settings?.status === "ready") {
    toneClass =
      "border-emerald-200 bg-emerald-50 text-emerald-800 dark:border-emerald-900 dark:bg-emerald-950/30 dark:text-emerald-200";
    statusLabel = "Plausible UTM reports are active for this project.";
    helpText =
      "When GA4 is not connected, the performance dashboard can use Plausible visits and pageviews grouped by utm_source, utm_medium, utm_campaign, utm_content, and utm_term.";
  } else if (settings?.status === "site_missing") {
    toneClass =
      "border-amber-200 bg-amber-50 text-amber-800 dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-200";
    statusLabel = "Add a Plausible site ID to turn on UTM reports.";
  } else if (settings?.status === "api_key_missing") {
    toneClass =
      "border-amber-200 bg-amber-50 text-amber-800 dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-200";
    statusLabel = "Plausible Stats API key still needs to be configured.";
    helpText =
      "Save the Plausible site ID here, then add PLAUSIBLE_API_KEY on the server so the dashboard can query Plausible.";
  }
    ...
    <div className="mb-4 space-y-3 rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
      ...
      >
        ...
        <div className="flex items-end gap-2">
          ...
          >
            {saving ? "Saving…" : trimmed ? "Save site" : "Clear site"}
          </button>
        </div>
      </form>
      ...
      {!valid && (
        <div className="text-xs text-red-600 dark:text-red-400">
          Use the site ID/domain from Plausible, without http:// or https://.
        </div>
      )}
      {settings?.apiBaseUrl && (
        <div className="text-xs text-zinc-500">
          Stats API base: {settings.apiBaseUrl}
        </div>
      )}
    </div>
  );
