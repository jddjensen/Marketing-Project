...
export function AuthForm({ mode }: { mode: Mode }) {
  ...
  const demoBypass =
    !isRegister && isLocalSupabase() && searchParams.get("bypass") !== "0";
      ...
      <div className="w-full max-w-sm">
        ...
        >
          ...
          <button
            type="submit"
            disabled={submitting}
            className="apple-tap w-full rounded-md bg-(--accent) px-4 py-2 text-sm font-medium text-white hover:opacity-90 disabled:opacity-60"
          >
            {submitting ? "…" : cta}
          </button>
        </form>
    ...
    <div className="flex min-h-screen items-center justify-center bg-zinc-50 px-6 text-zinc-900 dark:bg-zinc-950 dark:text-zinc-100">
      <div className="w-full max-w-md">
        <div className="modal-surface rounded-xl border border-zinc-200 bg-white p-8 text-center shadow-[var(--shadow-soft)] dark:border-zinc-800 dark:bg-zinc-900">
          <div
            className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-(--accent-soft) text-(--accent)"
            aria-hidden="true"
          >
            <svg
              viewBox="0 0 24 24"
          ...
          <ul className="mt-5 space-y-1.5 text-left text-[11px] text-zinc-500">
            <li>· The link is single-use and expires in 24 hours.</li>
            <li>
              · Spam folder is the usual culprit if it doesn&apos;t show up.
            </li>
            <li>· The wrong email? Just sign up again with the right one.</li>
          </ul>
          <Link
            href="/login"
            className="apple-tap mt-6 inline-block w-full rounded-md bg-(--accent) px-4 py-2 text-sm font-medium text-white hover:opacity-90"
          >
            Back to sign in
          </Link>
        </div>
