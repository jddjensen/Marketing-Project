...
export async function GET(request: NextRequest) {
  const storedState = decodeStateCookie(
  ...
  if (!storedState || storedState.nonce !== returnedState) {
    return redirectWithClearedCookie(
      request,
      redirectPath(projectId, "error", "state")
    );
  }
  ...
  if (providerError) {
    ...
    const safe = /^[a-zA-Z0-9_-]{1,64}$/.test(providerError)
      ? providerError
      : "provider";
    return redirectWithClearedCookie(
      request,
      redirectPath(projectId, "error", safe)
    );
  }
  ...
  if (!code) {
    return redirectWithClearedCookie(
      request,
      redirectPath(projectId, "error", "missing_code")
    );
  }
  ...
  if (!user) {
    return redirectWithClearedCookie(
      request,
      redirectPath(projectId, "error", "login")
    );
  }
  ...
  try {
    ...
    if (!refreshToken) {
      return redirectWithClearedCookie(
        request,
        redirectPath(projectId, "error", "refresh")
      );
    }
    ...
    if (error) {
      return redirectWithClearedCookie(
        request,
        redirectPath(projectId, "error", "save")
      );
    }

    return redirectWithClearedCookie(
      request,
      redirectPath(projectId, "connected")
    );
  } catch (error) {
    if (isGoogleAnalyticsError(error)) {
      return redirectWithClearedCookie(
        request,
        redirectPath(projectId, "error", "google")
      );
    }
    return redirectWithClearedCookie(
      request,
      redirectPath(projectId, "error", "unknown")
    );
  }
}
